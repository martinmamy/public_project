...
class ProblemForm(forms.ModelForm):
    ...
    def save(self, commit=True, user=None):
        ...
        instance.tags = ",".join([t.strip() for t in tags_str.split(",") if t.strip()])

        # Save post type from form
...
class AnswerForm(forms.ModelForm):
    class Meta:
        model = Answer
        fields = ["content", "file"]  # added 'file'
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["file"].required = False  # make file optional
...
class CommentForm(forms.ModelForm):
    class Meta:
        model = Comment
        # Only include 'content', problem/answer are set in the view
        fields = ["content"]
        widgets = {
