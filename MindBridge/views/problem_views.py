...
class CreateProblemView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if request.META.get("HTTP_ACCEPT") == "application/json":
            ...
            tags = data.get("tags", [])
            bounty_amount = float(data.get("bounty_amount", 0))
            ...
            # If bounty requested but PayPal not linked → block
            ...
            with transaction.atomic():
                ...
                problem = Problem.objects.create(
                    ...
                    category=category,
                    bounty_amount=bounty_amount if hasattr(request.user, "paypal_account") else 0,
                    tags=",".join(tags),
                    post_type=post_type
                )

                # Create bounty if PayPal linked
                ...
                mentioned_users = extract_mentions(description)
        ...
        bounty_amount = form.cleaned_data.get("bounty_amount", 0)
        post_type = form.cleaned_data.get("post_type", "issue")

        # Redirect if user wants bounty but hasn't linked PayPal
        if bounty_amount > 0 and not hasattr(request.user, "paypal_account"):
            messages.warning(
                request,
        ...
        with transaction.atomic():
            ...
            problem.tags = ",".join([t.strip() for t in tags_str.split(",") if t.strip()])
        ...
        # -----------------------
        if post_type == "information":
            return redirect("talenthub")  # Redirect to talenthub.html
...
class UpdateProblemView(LoginRequiredMixin, View):
    ...
    def post(self, request, problem_id):
        ...
        with transaction.atomic():
            ...
            if "tags" in data:
                problem.tags = ",".join([t.strip() for t in data["tags"] if t.strip()])
                updated = True

            if updated:
                problem.save()

                # Notify mentioned users if description changed
